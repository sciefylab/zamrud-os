//! Zamrud OS - BIP39 English Wordlist
//!
//! Official BIP39 wordlist with 2048 words.
//! Each word provides 11 bits of entropy (2^11 = 2048).
//! 12 words = 132 bits, 24 words = 264 bits.
//!
//! Reference: https://github.com/bitcoin/bips/blob/master/bip-0039/english.txt
//! License: Public Domain

/// Number of words in BIP39 wordlist
...
pub const WORDLIST: [WORDLIST_SIZE][]const u8 = .{
    // 0-63
...
/// Get word by index (returns empty string if out of bounds)
pub fn getWord(index: u16) []const u8 {
    if (index >= WORDLIST_SIZE) return "";
    return WORDLIST[index];
}

/// Find word index (returns null if not found)
pub fn findWord(word: []const u8) ?u16 {
    for (WORDLIST, 0..) |w, i| {
        if (strEq(w, word)) {
            return @intCast(i);
        }
